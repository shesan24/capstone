...
class GetActiveTransactionsApi(Resource):
    ...
    def getUserTransactions(self, userId):
        ...
        transactionsCursor = TransactionInfoCollection.find({"$and":[ {"userId":userId}, {"transactionState":"Initialized"}]})   #mongodb returns a cursor object, which we must iterate through, or else is not serializable 
